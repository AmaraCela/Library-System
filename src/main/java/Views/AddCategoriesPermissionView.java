...
public class AddCategoriesPermissionView extends BorderPane {
    ...
    public AddCategoriesPermissionView(Stage stage)
    {
        this.stage = stage;

        for(int i=0;i< RegisterStaffController.getManagers().size();i++)
        {
            if(((Manager)RegisterStaffController.getManagers().get(i)).isManageCategories())
            {
        ...
        Text revokeAddCategories = new Text("Revoke the permission of managing categories: ");
        Text allowAddCategories = new Text("Allow the permission of managing categories: ");

        revokeAddCategories.setFont(Font.font("Arial Rounded MT Bold",12));
        allowAddCategories.setFont(Font.font("Arial Rounded MT Bold",12));
        revokeAddCategories.setFill(Color.DARKBLUE);
        allowAddCategories.setFill(Color.DARKBLUE);

        GridPane gridPane = new GridPane();
        gridPane.add(revokeAddCategories,0,0);
        gridPane.add(allowAddCategories,1,0);
